<x-layouts.app title="Pengeluaran Kas">
    <div x-data="{ modalOpen: false }" class="space-y-6">
        
        {{-- Header --}}
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Pengeluaran Kas</h1>
                        ...
                        @forelse($transactions as $trx)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-right">
                                    <form action="{{ route('bendahara.transaction.destroy', $trx) }}" method="POST" onsubmit="return confirm('Hapus pengeluaran ini?');">
                                        @csrf
                                        @method('DELETE')
        ...
        <div x-show="modalOpen" 
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left shadow-xl transition-all"
                    ...
                    <form action="{{ route('bendahara.pengeluaran.store') }}" method="POST" class="space-y-5">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="amount" id="amount" required min="1"
                                    class="block w-full rounded-xl border-gray-300 pl-10 focus:border-red-500 focus:ring-red-500 py-2.5" placeholder="0">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="date" id="date" required value="{{ date('Y-m-d') }}"
                                class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 py-2.5">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" required rows="3"
                                class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 py-2.5" placeholder="Contoh: Beli Spidol, Sapu, Fotocopy, dll."></textarea>
                        </div>
                        ...
                        <div class="mt-6 flex justify-end gap-3">
                            ...
                            <button type="submit" 
                                    class="rounded-xl bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
                                Simpan Pengeluaran
                            </button>
                        </div>
                    </form>
